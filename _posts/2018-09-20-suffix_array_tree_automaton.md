...
![avatar](/img/in-post/dadc3.png)

> 多注意一些后缀数组越界和内存等的细节问题 <br>
...
### 2.2.3 回文子串
### 2.2.4 连续重复子串
* [POJ 3261](http://poj.org/problem?id=2406){:target="_blank"} <br>
给定一个字符串 L，已知这个字符串是由某个字符串 S 重复 R 次而得到的，求 R 的最大值。<br>
做法比较简单，穷举字符串 S 的长度 k，然后判断是否满足。判断的时候，先看字符串 L 的长度能否被 k 整除，再看 suffix(1)和 suffix(k+1)的最长公共前缀是否等于 n-k。在询问最长公共前缀的时候，suffix(1)是固定的，所以 RMQ问题没有必要做所有的预处理，只需求出 height 数组中的每一个数到height[rank[1]]之间的最小值即可。整个做法的时间复杂度为 O(n)<br>

* [SPOJ-REPEAT](https://www.spoj.com/problems/REPEATS/en/){:target="_blank"} <br>
一个字符串的某个子串是由某个子串重复而来的，找到这个重复的最大次数<br>
枚举子串长度L，然后求子串长度L的子串最多能连续出现几次。连续出现一次是肯定的，这里考虑至少两次的情况。假设子字符串S(如baa,aba)连续出现2次以上，那么在原串T(babbabaabaabaabab)中一定存在这样一种情况：枚举的一个长度L使得T(i), T(i+l), T(i+2*l), ... 中相邻的两个以上是重复串(baabaa)，那么重复次数用r = k/l+1, 其中k = lcp(i, i+1), 加一是为了加上自身。这时候我们还会注意到，从S[i+l]匹配到S[i+2 *l]的过程中，除了匹配到K/L个重复子串之外，还剩下了k%l个剩余字符，这也就是说在S[i+l]之前还有l-k%l个字符，如果这些字符也可以完成匹配的话，这就相当于利用多余的字符串还可以在匹配出一个重复子串，于是我们就还需要检查S[i+l - (l-k%l)]和S[i+ 2 *l-(l-k%l)]的lcp是否大于之前的匹配结果r。 
```cpp
for(int l = 1; l < n; l++)
{
